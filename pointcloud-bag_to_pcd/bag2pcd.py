...
def convert_pointcloud2_to_o3d(ros_cloud):
    ...
    if 'rgb' in fields or 'rgba' in fields:
        color_field = 'rgb' if 'rgb' in fields else 'rgba'
        ...
        for point in cloud_points:
            x, y, z, color = point
            # 兼容 float/int 类型的RGB字段
            if isinstance(color, float):
                # ROS标准/RealSense一般用小端字节序
                i = struct.unpack('<I', struct.pack('<f', color))[0]
            else:
                i = int(color)
            r = (i >> 16) & 0x0000ff
            g = (i >> 8) & 0x0000ff
        ...
        points = [[x, y, z] for x, y, z in cloud_points]
        o3d_cloud = o3d.geometry.PointCloud()
        o3d_cloud.points = o3d.utility.Vector3dVector(points)
    return o3d_cloud
