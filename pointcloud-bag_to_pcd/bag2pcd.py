...
def extract_and_merge_pointclouds_from_bag():
    ...
    # 参数设置
    bag_file = "/home/sax/2025-07-08-17-42-46.bag"
    ...
    for topic, msg, t in bag.read_messages(topics=[topic]):
        if msg._type == 'sensor_msgs/PointCloud2':
            try:
                ...
                processed_count += 1
                print(f"Processed frame {processed_count}/{frame_count}")
    ...
    if processed_count > 0:
        filename = os.path.join(output_path, "merged_pointcloud.pcd")
        o3d.io.write_point_cloud(filename, merged_cloud)
        print(
            f"Successfully merged {processed_count} point clouds to {filename}")
    else:
